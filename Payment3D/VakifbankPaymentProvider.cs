        ...
        public async Task<PaymentGatewayResult> ThreeDGatewayRequest(PaymentGatewayRequest request)
        {
            try
            {
                ...
                var httpParameters = new Dictionary<string, string>
                {
                    ...
                    { "ExpiryDate", $"{string.Format("{0:00}", request.ExpireYear)}{string.Format("{0:00}", request.ExpireMonth)}" },
                    { "PurchaseAmount", request.TotalAmount.ToString("N2", new CultureInfo("en-US")) },
                    { "Currency", request.CurrencyIsoCode },//TL 949 | EURO 978 | Dolar 840
                    ...
                    */
                    { "BrandName", "100" },
                    { "VerifyEnrollmentRequestId", request.OrderNumber },//sipariş numarası
                    { "SessionInfo", "1" },//banka dökümanları sabit bir değer
                    { "MerchantID", merchantId },
                    { "MerchantPassword", merchantPassword },
                    { "SuccessUrl", request.CallbackUrl.ToString() },
                    { "FailureUrl", request.CallbackUrl.ToString() }
                };
