    ...
    public class PosnetPaymentProvider : IPaymentProvider
    {
        ...
        public async Task<PaymentGatewayResult> ThreeDGatewayRequest(PaymentGatewayRequest request)
        {
            ...
            try
            {
                ...
                {
                    { "posnetData", data1Node.InnerText },
                    { "posnetData2", data2Node.InnerText },
                    { "digest", signNode.InnerText },
                    ...
                    { "vftCode", string.Empty },

                    { "merchantReturnURL", request.CallbackUrl },//geri dönüş adresi
                    { "lang", request.LanguageIsoCode },
                    { "url", string.Empty },//openANewWindow 1 olarak ayarlanırsa buraya gidilecek url verilmeli
                    { "openANewWindow", "0" },//POST edilecek formun yeni bir sayfaya mı yoksa mevcut sayfayı mı yönlendirileceği
                    { "useJokerVadaa", "1" }//yapıkredi kartlarında vadaa kullanılabilirse izin verir
                };
